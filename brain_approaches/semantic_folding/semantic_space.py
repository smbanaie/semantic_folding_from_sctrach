...
def build_semantic_graph(matrix: scipy.sparse.csr_matrix,
    ...
    for i in tqdm(range(len(context_ids)), desc="Building graph"):
        for j in range(i + 1, len(context_ids)):
            ...
            similarity = submatrix[i].dot(submatrix[j].T).toarray()[0][0]
...
def compute_force_layout(G: nx.Graph, grid_size: int) -> Dict[str, Tuple[float, float]]:
    ...
    pos = nx.spring_layout(
        ...
        k=grid_size / 10,  # Optimal distance between nodes
...
def main():
    ...
    context_texts = load_corpus_contexts(corpus_path)

    # Build semantic graph
    G = build_semantic_graph(matrix, metadata, args.edge_threshold, args.max_edges)
