...
=== O. SECOND LEARNED SPARSE MODEL: SPLADE-v3 (reviewer #18) ============
Purpose : close review #18 ("second learned sparse model is non-negotiable")
          - show the SF+SPLADE operator findings are not an artifact of one
          specific SPLADE checkpoint.
Model   : naver/splade-v3  (gated HF repo)
          vs baseline naver/splade-cocondenser-ensembledistil ("v2")

BLOCKER + FIX (important for reproducibility):
  splade-v3 is GATED. First attempt failed: loader used
    from_pretrained(..., token=False)  # forces ANONYMOUS access
  which refuses auth even when logged in. Fixed in
  semantic_folding/splade_scorer.py lines 86-87: token=False -> token=None
  (lets huggingface_hub use stored credentials; `huggingface_hub.login()`
  run once locally with the owner's token; token NOT stored in any file).
  Verified ad hoc (PASS, exit 0): no token=False remains; gated config.json
  downloads under login; public-model tokenizer regression clean.

CACHE-ISOLATION PITFALL (avoided):
  SPLADEScorer caches corpus vectors at <run_dir>/splade_corpus_vectors.npy
  with NO model name in the filename. Running v3 on the original run dirs
  would SILENTLY LOAD STALE v2 VECTORS. Mitigation: cloned fresh run dirs
  (hotpotqa: runs/run_v3_smoke, musique: runs/run_v3_full), deleted any
  .npy before launching so v3 encoded its own vectors.

RUNS (all BENCH_OK, n=50, 7 operators, SF+signal-B pair):
- hotpotqa v3: outputs/hotpotqa_benchmark/benchmarks/benchmark_20260823_195752
- musique  v3: outputs/musique_benchmark/benchmarks/benchmark_20260823_201111

RESULTS (MRR, n=50; v2 = cocondenser, v3 = splade-v3):
              | HotpQA v2 | HotpQA v3 | MuSiQue v2 | MuSiQue v3 |
...
FINDING: operator ORDERING is stable across checkpoints. CombSUM first on
both datasets under both models; v3 slightly lifts score-space ops;
magnitude-vs-rank separation persists. The effect is a property of the
PAIRING (SF spatial-magnitude x log1p-pooled learned sparse signal), not of
one checkpoint.

Paper integration:
- new section 6.5.2 "Second Learned Sparse Checkpoint (SPLADE-v3)" with the
  full comparison table and checkpoint-robustness reading
- old 6.5.1 (alpha sensitivity) renumbered -> 6.5.3; internal cross-refs updated
- appendix_stats/splade_v3_comparison.md holds the raw table
- comparison generated by temp/splade_v3_compare.py (reads op_*/summary.json)

=== P. DEEP-POOL BM25+SPLADE N-SWEEP (reviewer #22/#23, closes section 8.4b) ===
Purpose : the paper's 8.4 N-sweep previously covered only SF+SPLADE and said
          "BM25+SPLADE HotpotQA N-sweep remains future work". Delivered.
Design  : signal A = BM25 (--signal-a bm25), signal B = SPLADE-cocondenser,
          hotpotqa n=10 queries per N, ALL SEVEN operators (the old SF sweep
          reported only 3), each N a freshly built padded pool via
          generic_benchmark all --deep-pool N.
Runs (all ALL_OK):
  N=20  -> benchmark_20260823_214627 (index run_20260823_214453)
  N=50  -> benchmark_20260823_215235 (index run_20260823_215059)
  N=100 -> benchmark_20260823_220028 (index run_20260823_215901)
  N=494 -> benchmark_20260823_220728 (index run_20260823_220549)
CONFIG CHECK (approved read-only verification): each index config.yml carries
  deep_pool: 20/50/100/494 respectively, and query_doc_map.json shows pool
  sizes exactly min=avg=max = 20/50/100/494 -> four DISTINCT pools were built.

RESULTS (MRR, flat at every N):
              linear  rrf  borda  combsum combmnz zscore minmax
  all N        0.900 0.850 0.850   0.950   0.950   0.950  0.900
(P@1: 0.800 rank-only / 0.900 magnitude family at every N.)
Extractor: temp/nsweep_extract.py ->
  docs/papers/Journal A/appendix_stats/deep_pool_nsweep.md
Verified ad hoc (PASS): both extractors deterministic (byte-identical .md on
rerun); 84 table cells cross-checked against raw summary.json.

INTERPRETATION (now in paper section 8.4 as two-part (a)/(b) with Reading):
- SF+SPLADE (a): CombSUM flat 1.000; RRF swings 0.667->0.883->0.783.
- BM25+SPLADE (b): everything flat in N; magnitude family 0.950 vs
  rank-only 0.850 - a compressed but non-inverted gap.
=> Pool size / score concentration does NOT separate operators; signal-A
score geometry does. Consistent with 6.5 cross-pair finding + 7.4 causal
perturbation. Closes the "future work" placeholder in old 8.4.
