...
**Status (honest):** This is the first true full-corpus reranking experiment in the paper — prior draft §8.5 had only *planned* future work for a `convert_to_full_corpus_format()` sidecar per adapter. We had one for BEIR (adapter) but none for the QA adapters. We generated the `hotpotqa_corpus.txt` sidecar (494 docs) from the converted jsonl and ran the benchmark. The open question the full-corpus run would close is whether SF's *first-stage recall* becomes the binding constraint at corpus scale — i.e. whether the §6 findings (operator selection effect, score-geometry dependence) survive a real deep-pool with millions of distractors, or whether they hold only at the reranking stage. We explicitly flag this as the single most important validation still outstanding (§10, future work). The combsum MRR=1.000 over 494 docs is not a retrieval-recall artifact; it is genuine quality at the reranking stage, but we do not claim it transfers to first-stage corpus retrieval at MS MARCO scale — that requires a dedicated deep-pool infrastructure and is deferred.
