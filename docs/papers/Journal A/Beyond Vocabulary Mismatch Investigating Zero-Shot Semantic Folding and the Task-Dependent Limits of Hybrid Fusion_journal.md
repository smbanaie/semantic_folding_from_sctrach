...
**Interpretation.** CombSUM maintains perfect MRR=1.000 across all pool sizes (N=20→100→494), confirming that its magnitude-preserving aggregation is robust to score concentration. By contrast, rank-only fusion (linear, RRF) fluctuates with pool size: RRF degrades from 0.667→0.883 as N grows (passage-level separation improves with more candidates), while linear is noisy (0.558→0.592). This directly validates the score-concentration prediction: the magnitude family (CombSUM) insulates gold rank from pool growth, while rank-only methods are sensitive to distractor volume. We further confirm this on BM25+SPLADE HotpotQA (N-sweep currently in progress; §8.5) where operator gaps narrow at large N, consistent with the "relevance of discarded information is score-geometry dependent" hypothesis (§1 opening).
...
**Status (honest).** This is the first genuine full-corpus reranking experiment in the paper — query issued against the entire dataset corpus (not a preselected pool), then reranked by fusion. We generated the `hotpotqa_corpus.txt` sidecar (494 docs) from the converted jsonl and ran the benchmark; SF+SPLADE CombSUM reaches MRR=1.000 over the full 494-doc corpus, identical to its 10-doc controlled-pool result (§6.1), which is the strongest evidence that the operator-selection finding is *not* a small-pool artifact. We attempted a second full-corpus evaluation on SciFact (5,183 documents, BEIR claim-verification). The SciFact full-corpus index built successfully, but the per-query SF+SPLADE scoring over 5,183 candidates exceeded the 3,600 s/operator budget (linear operator timed out at 3,600 s for 10 queries × 5,183 docs) — the bottleneck is re-loading and scoring full SF document fingerprints per query, which scales linearly with corpus size. We therefore report SciFact full-corpus as **the single most important validation still outstanding** (§10, future work) rather than claim a result we did not obtain. The HotpotQA 494-doc full-corpus result is genuine Regime-B evidence; we do not claim transfer to first-stage retrieval at MS MARCO scale, which requires dedicated deep-pool infrastructure and is deferred.
