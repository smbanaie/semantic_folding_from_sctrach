...
This leads to our primary theoretical contribution: **The Operator-Topology Constraint**. The optimal fusion operator for a hybrid retrieval system is a strict mathematical function of task complexity. For single-hop semantic matching, rank-level fusion (RRF) is strictly dominant due to scale invariance. For multi-hop compositional reasoning, score-level fusion (Linear) is strictly dominant to preserve magnitude-encoded confidence signals. Treating fusion operators as interchangeable hyperparameters is a mathematical error.
...
1.  **The Operator-Topology Constraint:** We mathematically formalize why rank-level fusion (RRF) is strictly superior for single-hop tasks, while score-level fusion (Linear) is strictly superior for multi-hop tasks. We prove that destroying absolute score magnitudes destroys the compositional confidence required for multi-hop reasoning.
