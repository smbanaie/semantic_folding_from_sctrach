...
4. **A comprehensive empirical framework establishing the boundary conditions of unsupervised sparse retrieval across 8 diverse datasets, including SciFact.** Through three configurations (SF-only, SPLADE-only, SF+SPLADE) we characterize precisely where unsupervised semantic folding adds value and where it does not. On SciFact (~5,000 documents), SF-only achieves MRR 0.755, matching a fully trained DPR model (0.675) without any training data — establishing the zero-shot niche. We provide practitioners with an evidence-based deployment guide.

5. **The discovery of the Feature-Invariance Principle**, proving that internal SDR architectural modifications (cross-attention, snippet ranking, adaptive spreading) yield diminishing returns, thereby directing future research toward external signal fusion rather than incremental SDR refinement.

6. **The Complementarity Illusion**: a formal diagnostic framework demonstrating that structurally different retrieval methods (spatial clustering vs. learned sparse expansion) do not necessarily provide complementary gains. We establish a Kendall's $\tau$ threshold ($\tau_c \approx 0.80$) below which linear fusion is beneficial — a general methodological contribution applicable beyond Semantic Folding to any hybrid retrieval system.
