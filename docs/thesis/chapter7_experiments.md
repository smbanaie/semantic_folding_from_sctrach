...
|---------|--------|:-------:|------|:-------------------:|--------|
| **PopQA** | Entity Lookup | 50 | Wikidata entity retrieval | 2/query | Mallen et al. (2023) |
| **NarrativeQA** | Narrative Comprehension | 50 | Script comprehension | 1/query | Kočiský et al. (2018) |
...
**Selection rationale**: These nine datasets were chosen to span the full spectrum of retrieval difficulty — from simple entity lookup (PopQA, perfect MRR achievable) through narrative comprehension and reading comprehension to multi-hop compositional reasoning (MuSiQue, 2Wiki, HotpotQA) and dense biomedical retrieval (BioASQ). This coverage allows us to identify precisely which task characteristics Semantic Folding handles well and which expose its limitations.
...
*\\*BioASQ BM25 from published baselines (Nentidis et al., 2025). †NarrativeQA: AP=0.017 — small pools inflate MRR. SPLADE-only benchmarks run 2026-06-29 with α=0.0 (100% SPLADE).*
...
2. **SF+SPLADE matches BM25 on HotpotQA** (0.857 vs 0.869, −1.4%) — closing the gap to within statistical noise
3. **SPLADE is the only feature that consistently improves SF** — all other features (cross-attention, snippet, adaptive spreading, learned grid, ontology expansion, query decomposition) either degrade or have zero effect
4. **L2 normalization improves SF by +4.0%** — sqrt(nnz) penalizes longer documents unfairly
5. **t-SNE perplexity=50 improves both Belebele and PubMedQA** — better local clustering for discrimination
6. **Zero-shot superiority on SciFact** (0.755 vs DPR 0.675) — validates unsupervised semantic matching for scientific claim verification
7. **FAISS reduces OOV expansion by 400×** — enabling practical deployment of OOV handling
8. **NoOOV ablation**: Verified across 6 datasets that OOV expansion has zero effect — always safe to disable

**Collective validation of hypotheses**:

| Hypothesis | Evidence Grade | Key Supporting Datasets | 
|------------|:-------------:|------------------------|
| H1 (Semantic Matching) | Strong (partial) | MuSiQue +92%, BioASQ −69% — supported only in controlled pools |
| H2 (Complementarity) | Strong | SF+Splade beats SF-only on 7/9 datasets; avg gain +11.8% |
| H3 (Feature Invariance) | Conclusive | All 7 tested features except SPLADE ≤ 0% effect |

The experiments collectively demonstrate that: (a) SF's semantic matching is real and measurable, but bounded by pool size; (b) SPLADE is the only feature that adds genuinely complementary signal; and (c) the SF pipeline is remarkably resistant to architectural improvements, with all 7 non-SPLADE variants failing to improve performance.

### 7.4.2 Best Configuration Summary

| Parameter | Value | Rationale |
|-----------|-------|-----------|
| **Base method** | SF+Splade | Best for 7/9 datasets |
| Dimensionality reduction | **UMAP (n_neighbors=15, min_dist=0.0)** | **Matches or beats t-SNE on 7/9 datasets; avg +1.3% MRR; 10× faster** |
| t-SNE perplexity | 50 | +4% vs p=30 (t-SNE fallback) |
| Grid size | 64 | Optimal density (5-15%) |
| Doc normalization | L2 | +4% vs sqrt_nnz |
| Spreading | radius=1, decay=0.5 | Optimal for 64×64 grid |
| Top percent | 0.10 | 5% loses signal, 15% adds noise |
| Gaussian smoothing | σ=1.5 | Critical (σ=0 → −31%) |
| Weighting | IDF | −0.86% vs uniform (keep IDF) |
| Morton encoding | True | Z-order spatial encoding |
| OOV expansion | Disabled (--no-oov) | Zero effect, saves memory |
| Feature variants | None | All tested variants degrade or have zero effect |

### 7.4.3 Thesis Positioning

> "Semantic Folding, when combined with SPLADE, achieves the strongest retrieval performance among unsupervised methods on the MuSiQue multi-hop QA dataset — outperforming both BM25 (+92%) and dense retrieval methods (+7.2% vs HippoRAG2) — while requiring zero training data. On 7 of 9 benchmark datasets, SF+Splade produces MRR ≥ 0.857. These results validate the hypothesis that combining unsupervised semantic matching via topographic grid encoding with learned sparse expansion provides a retrieval architecture that is simultaneously competitive, interpretable, and training-free."

---

## 7.5 Reproduction

**Default configuration** (as of 2026-06-29):
...
Feature variants (not recommended):
```
generic_benchmark.py all --dataset 2wikimultihopqa --jsonl data/2wikimultihopqa/converted/2wikimultihopqa.jsonl --cross-attention
generic_benchmark.py all --dataset 2wikimultihopqa --jsonl data/2wikimultihopqa/converted/2wikimultihopqa.jsonl --snippet
generic_benchmark.py all --dataset 2wikimultihopqa --jsonl data/2wikimultihopqa/converted/2wikimultihopqa.jsonl --adaptive-spreading
```

BM25 baseline:
```
bm25_benchmark.py --dataset <name> --jsonl data/<name>/converted/<name>.jsonl
```
