...
5. **Legal and financial domain failures**: SF achieves MRR=0.000 on MAUD (100 queries, legal document review) — complete retrieval failure. DocFinQA (MRR=0.250) and DROP (MRR=0.320) also show severe degradation. These domains require capabilities beyond phrase-level semantic matching: numerical reasoning (DROP, DocFinQA), and conditional logic evaluation (MAUD). BM25 also struggles on legal tasks.ggles on CUAD (MRR=0.244), confirming that these are inherently difficult retrieval tasks. SF's zero-score on legal tasks represents a known boundary: structural reasoning is orthogonal to the semantic matching paradigm.
