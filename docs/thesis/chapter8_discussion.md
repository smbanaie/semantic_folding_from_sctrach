...
Our evaluation of Semantic Folding across 13 benchmark datasets reveals a clear performance hierarchy that maps onto task characteristics. The pattern is striking: SF performance degrades linearly with the number of reasoning hops required.
...
5. **Legal and financial domain failures**: SF achieves MRR=0.000 on both CUAD (200 queries, legal contract clause extraction) and MAUD (100 queries, legal document review) — complete retrieval failure. DocFinQA (MRR=0.250) and DROP (MRR=0.320) also show severe degradation. These domains require capabilities beyond phrase-level semantic matching: numerical reasoning (DROP, DocFinQA), clause cross-referencing across document sections (CUAD), and conditional logic evaluation (MAUD). BM25 also struggles on CUAD (MRR=0.244), confirming that these are inherently difficult retrieval tasks. SF's zero-score on legal tasks represents a known boundary: structural reasoning is orthogonal to the semantic matching paradigm.

6. **Discrete reasoning gap**: DROP (MRR=0.320) demonstrates that SF cannot perform counting, sorting, or comparison operations. These require aggregating information across passages — a task that depends on compositional reasoning, not semantic similarity. Even with L2 normalization (+14.3% on DROP), the performance remains far below BM25 (0.762).

7. **Computational cost**: SF indexing takes ~10 minutes for 100 queries (vs ~10 seconds for BM25). Per-query scoring takes ~47s steady-state (dominated by SPLADE inference). The OOV expansion step, previously the bottleneck (~30s per query), has been optimized to ~0.075s using FAISS approximate nearest neighbor search.
