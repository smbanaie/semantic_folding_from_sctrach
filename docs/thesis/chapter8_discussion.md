...
Our evaluation of Semantic Folding across 11 benchmark datasets reveals a clear performance hierarchy that maps onto task characteristics. The pattern is striking: SF performance degrades linearly with the number of reasoning hops required.
...
5. **Financial and discrete reasoning failures**: DocFinQA (MRR=0.250) and DROP (MRR=0.320) show severe degradation. These domains require capabilities beyond phrase-level semantic matching: numerical reasoning (DROP, DocFinQA), and conditional logic evaluation.
