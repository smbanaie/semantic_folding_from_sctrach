...
13. [Reranker Training Tools](#reranker-training-tools)
...
| `compute_dice_coefficient` | `vec1: np.ndarray, vec2: np.ndarray` | `float` ∈ $[0, 1]$ |
| `compute_overlap_coefficient` | `vec1: np.ndarray, vec2: np.ndarray` | `float` ∈ $[0, 1]$ |
| `compute_binary_jaccard` | `vec1: np.ndarray, vec2: np.ndarray` | `float` ∈ $[0, 1]$ |
| `compute_idf_weighted_intersection` | `vec1: np.ndarray, vec2: np.ndarray, idf_weights: np.ndarray` | `float` ∈ $[0, 1]$ |
...
| `batch_compute_binary_similarities` | `query_fp: csr_matrix, doc_fps: List[csr_matrix], metric: str, idf_weights: Optional[np.ndarray]` | `np.ndarray` — scores per doc |
...
$$D(A,B) = \frac{2|A \cap B|}{|A| + |B|} \qquad O(A,B) = \frac{|A \cap B|}{\min(|A|, |B|)}$$
$$S_{\text{idf}}(A,B) = \frac{\sum_{i \in A \cap B} w_i}{\sum_{i \in A} w_i}$$

All handle zero vectors (returns `0.0`). `compute_cosine_similarity` accepts sparse matrices. Binary metrics (`compute_dice_coefficient`, `compute_overlap_coefficient`, `compute_binary_jaccard`) extract active bit positions from sparse matrices and compute set-based similarity. `batch_compute_binary_similarities` vectorizes binary metric computation for efficiency.
...
Located in `semantic_folding/tools/`. These tools implement the LambdaMART cascade re-ranking pipeline.

| Tool | Purpose | Usage |
|------|---------|-------|
| `generate_training_data.py` | Extract labeled features from benchmark runs | `python -m semantic_folding.tools.generate_training_data --run-dir <run> --jsonl <jsonl> --output <features.jsonl>` |
| `train_model.py` | Train LambdaMART on extracted features | `python -m semantic_folding.tools.train_model --features <features.jsonl> --output <model.txt>` |
| `create_query_jsonl.py` | Map raw dataset queries to run indices | `python -m semantic_folding.tools.create_query_jsonl --raw-jsonl <raw> --run-dir <run> --output <queries.jsonl>` |

### Feature Vector Format (35 features)

Each feature vector contains:
- **Binary similarity** (5): Jaccard, Dice, overlap, Hamming, cosine
- **Asymmetric** (3): Containment, coverage, IDF-weighted intersection
- **Bit-density** (8): popcount(Q), popcount(D), intersection, union, mismatch, density(Q), density(D)
- **Block histogram** (16): Per-block Jaccard for 16 blocks of 256 bits
- **Auxiliary** (3): BM25 score, query length, document length

### Training Workflow

```bash
# Step 1: Generate features
python -m semantic_folding.tools.generate_training_data \
  --run-dir outputs/belebele_benchmark/runs/run_<ts> \
  --jsonl data/belebele/converted/belebele.jsonl \
  --output temp/features.jsonl

# Step 2: Train model
python -m semantic_folding.tools.train_model \
  --features temp/features.jsonl \
  --output temp/model.txt

# Step 3: Evaluate cascade
python -m semantic_folding.dataset_benchmark.generic_benchmark benchmark \
  --dataset belebele --run-dir <run> --jsonl <jsonl> \
  --rerank --rerank-model temp/model.txt
```

### Dependencies

- `lightgbm >= 4.0` (LambdaMART implementation)
- `scikit-learn`, `numpy`, `scipy` (already project dependencies)