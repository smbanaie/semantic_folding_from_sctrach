...
The pipeline supports multiple similarity metrics, configurable via `--sim-metric`. The default is cosine similarity with configurable document normalization.

#### 2.1.1 Cosine Similarity (Default)

Given a query fingerprint $\mathbf{q} \in \mathbb{R}^{g^2}$ (float-valued, IDF-weighted, L2-normalised) and a document fingerprint $\mathbf{d}_i \in \mathbb{R}^{g^2}$, the score is:

\[
\text{score}(Q, D_i) = \frac{ \mathbf{q} \cdot \mathbf{d}_i^\mathsf{T} }{ \|\mathbf{q}\|_2 \cdot \phi(\mathbf{d}_i) }
\tag{1}
\]

where $\phi(\mathbf{d}_i)$ is the document normalization factor:

| `--doc-norm` | $\phi(\mathbf{d}_i)$ | Effect |
|-------------|----------------------|--------|
| `sqrt_nnz` | $\sqrt{\|\mathbf{d}_i\|_0}$ | Favors longer documents |
| `l2` (adopted) | $\|\mathbf{d}_i\|_2 = \sqrt{\sum_j d_{ij}^2}$ | Standard cosine |
| `l1` | $\|\mathbf{d}_i\|_1 = \sum_j |d_{ij}|$ | Aggressive penalization |
| `max` | $\max_j d_{ij}$ | Normalizes by peak activation |

After L2 normalisation of the query, $\|\mathbf{q}\|_2 = 1.0$.

**Implementation:** `query_processor.py:1644-1673`
...
#### 2.1.2 Binary Set Similarity Metrics

For binary sparse vectors, cosine similarity has limitations: ignores bit position, sparsity bias, loses asymmetry (Bajaj et al., 2018). Alternative metrics via `--sim-metric`:
...
where $\mathcal{A} = \{i : q_i > 0\}$, $\mathcal{B} = \{i : d_i > 0\}$. Biased toward the smaller set (query).
...
Maximum robustness to set size differences. Returns 1.0 when query is a subset of document.
...
Excludes shared absences ($M_{00}$) — appropriate for sparse binary data (Broder, 1997).
...
Weights rare concepts more than common ones (Sparck Jones, 1972).

#### 2.1.3 Asymmetric Scoring

Standard set similarity is symmetric. Retrieval is inherently asymmetric. Via `--asymmetric`:
...
**Combined**:
\[
S_{\text{asym}}(\mathbf{q}, \mathbf{d}) = \alpha \cdot S_{\text{contain}} + (1 - \alpha) \cdot S_{\text{cover}}
\tag{8}
\]
...
Via `--score-norm`, applied after scoring and before ranking:
