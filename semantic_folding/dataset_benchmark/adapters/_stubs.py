...
class CuadAdapter(_StubAdapter):
    dataset_name = "cuad"
    display_name = "CUAD"
    ...
    def download(self, output_dir: Path) -> Path:
        ...
        if not f.exists():
            raise FileNotFoundError(
                f"CUAD raw data not found at {f}. "
                f"Download from huggingface.co/datasets/contractualai/cuad and place there."
            )
        return output_dir
    ...
    def convert_to_musique_format(
        ...
        output_dir = Path(output_dir)
        output_dir.mkdir(parents=True, exist_ok=True)
        out_path = output_dir / "cuad.jsonl"

        src = raw_path / "test.json"
        if not src.exists():
            raise FileNotFoundError(f"CUAD test.json not found: {src}")

        with open(src, encoding="utf-8") as f:
            data = json.load(f)

        qa_pairs = []
        for ci, doc in enumerate(data["data"]):
            for qi, qa in enumerate(doc["paragraphs"][0]["qas"]):
                if not qa.get("is_impossible", False) and qa.get("answers"):
                    qa_pairs.append((ci, qi))
        ...
        all_ci = list(contracts.keys())
        entries = []
        for ci, qi in qa_pairs:
            doc = data["data"][ci]
            ...
            distractor_cis = [c for c in all_ci if c != ci]
            random.shuffle(distractor_cis)
            distractors = distractor_cis[:19]

            paragraphs = [{
                "idx": 0,
                "title": f"contract_{ci}",
                "paragraph_text": gold_ctx,
                "is_supporting": True,
            }]
            for i, dci in enumerate(distractors):
                paragraphs.append({
                    "idx": i + 1,
                    "title": f"contract_{dci}",
                    "paragraph_text": contracts[dci],
                    "is_supporting": False,
                })
        ...
        with open(out_path.with_suffix(".stats.json"), "w") as f:
            json.dump(stats, f, indent=2)
        print(f"CUAD: wrote {len(entries)} queries -> {out_path}")
        self._converted_path = out_path
        return out_path
