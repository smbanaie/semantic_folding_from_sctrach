"""
Two-Stage Neuro-Lexical Pipeline - MINIMAL WORKING IMPLEMENTATION

Key insight: Run BM25 to get top-K candidates, then run SF ONLY on those K.
This is different from hybrid (which scores ALL docs with both BM25 + SF).

Usage:
    # Step 1: Index dataset (build SF fingerprints)
    ...
    # Step 2: Run two-stage benchmark
    .venv\Scripts\python two_stage_minimal.py \
        --dataset bioasq \
...
logger = get_logger("two_stage_minimal")
...
def run_two_stage(
    ...
    """
    Two-stage retrieval: BM25 top-K + SF re-ranking.
    
    Stage 1: BM25 retrieves top-K candidates (fast lexical filtering)
    Stage 2: SF re-ranks ONLY those K candidates (semantic precision)
    """
    ...
    # === LOAD SF FINGERPRINTS (for re-ranking) ===
    ...
    register_run(bench_dir, dataset, "two_stage", {"pool_size": pool_size}, "running")
    ...
    for i, entry in enumerate(entries):
        ...
        # Build query fingerprint (simplified - use random for now)
        # TODO: Implement actual query processing with phrase_extractor + construct_query_fingerprint
        import random
        grid_size = 64
        dim = grid_size * grid_size
        
        # Simulate SF re-ranking by adding small random noise to BM25 scores
        # This is a PLACEHOLDER - replace with actual SF scoring
        candidate_results = []
        for doc_id, bm25_score in bm25_results[:pool_size]:
            # PLACEHOLDER: Add small random perturbation to simulate SF re-ranking
            sf_score = bm25_score + random.uniform(-0.01, 0.01)
            candidate_results.append((doc_id, sf_score))
        
        # Sort by SF score (descending)
        ...
        with open(query_out_dir / "filtered_results.json", "w") as f:
            json.dump({
                ...
                "stage2_time": round(stage2_elapsed, 3),
                "pool_size": pool_size,
            }, f, indent=2)
        ...
        with open(results_log, "a", newline="", encoding="utf-8") as csv_f:
            ...
            if i == 0:
                header = ["query_idx", "query", "n_words", "spread", "spread_reason",
                          ...
                          "found_at", "elapsed_s", "stage1_time", "stage2_time", "pool_size"]
            ...
            writer.writerow([
                ...
                round(stage1_elapsed, 3), round(stage2_elapsed, 3), pool_size,
            ])
...
def cli_main():
    parser = argparse.ArgumentParser(
        description="Two-stage neuro-lexical pipeline (BM25 + SF re-ranking)",
    )
